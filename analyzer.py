        ...
        for file in files:
            if file.endswith(".py"):
                ...
                with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                    ...
                    # Look for repeating substrings (multisymbols)
                    for length in range(2, max_pattern_len + 1):
                        for i in range(len(content) - length + 1):
                            pattern = content[i:i+length]
                            # Count patterns that aren't purely whitespace
                            if pattern.strip():
                                patterns[pattern] += 1
    
    # Prioritize (length * frequency) to maximize representational savings
    most_valuable = sorted(patterns.items(), key=lambda x: len(x[0]) * x[1], reverse=True)
    
    return [p[0] for p in most_valuable[:top_n]]

if __name__ == "__main__":
    # Based on your file structure, point to the source folder of requests
    ...
    if common_patterns:
        # Crucial step: Save the 'Knowledge Base' to disk
